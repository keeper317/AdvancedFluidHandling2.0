...
local function build_valve_picture(valve)
  local file_path = "__advanced-fluid-handling-space-age__/graphics/entity/valves/"
  local pictures = {
    picture = {
      north = {
        layers = {
...
for num, type in pairs(types) do
  for valve, datas in pairs(valve_table) do
    for percent, stat in pairs(datas.percents) do
      local current_valve = util.table.deepcopy(data.raw["pump"]["pump"])
